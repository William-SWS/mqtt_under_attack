...
for cell in nb['cells']:
    if cell['cell_type'] == 'code':
        ...
        # 1. Replace X.fillna(0) with train_test_split and SimpleImputer (median)
...
"""
        ...
        source = source.replace('../reports_refactored/', '../reports/reports_refactored_median/')
        source = source.replace("Path('../models/models_optuna')", "Path('../models/models_refactored_strategy_median/optuna')")
        source = source.replace("Path('../models/models_refactored')", "Path('../models/models_refactored_strategy_median/base')")
        
        # 3. Rename final reports if they exist
        source = source.replace('optuna_by_selector_comparison.csv', 'optuna_by_selector_comparison_cenario2_median.csv')
        
        # We need to clean up splitting and make sure \n is properly retained
        cell['source'] = [line + ('\n' if not line.endswith('\n') else '') for line in source.split('\n') if line]

with open('notebooks_refactored/03_dos_complete_pipeline.ipynb', 'w') as f:
