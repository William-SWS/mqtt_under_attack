#!/usr/bin/env python3
"""
Gráfico de barras: custo de memória (delta) por cenário de teste.

Lê api_inference/results_inference/*.json e gera um PNG com:
  - Eixo X: cenários de teste (t01..t16, em ordem cronológica)
    com concorrência e OMP_NUM_THREADS por cenário
  - Eixo Y: memória usada (MB) = memory_after − memory_before
  - Barras coloridas por nível de concorrência (legenda)
  - Apenas execuções que benchmarkaram modelos LowVariance e têm
    medições de memória válidas
"""
...
DEFAULT_OUTPUT = (
    PROJ_ROOT / "api_inference" / "results_inference" / ("memory_delta_by_scenario.png")
)
...
# t01, t02, t14, t15, t16: valor não documentado no git (t01/t02/t16 informados
# manualmente).
OMP_THREADS_BY_RUN = {
    "t01": 5,
    "t02": 5,
...
def load_runs(input_dir: Path) -> tuple[pd.DataFrame, list[str]]:
    ...
    for json_path in sorted(input_dir.glob("*.json")):
        ...
        mem_before = (data.get("system_before") or {}).get("memory_used_mb")
        ...
        if mem_before is None or mem_after is None:
            excluded.append(f"{run_id}: sem medições de memória válidas")
            continue
        ...
        rows.append(
            {
                ...
                "delta_mb": float(mem_after) - float(mem_before),
...
def plot_bars(df: pd.DataFrame, output_path: Path) -> None:
    ...
    bars = ax.bar(df["run"], df["delta_mb"], color=colors)
    ...
    fig.suptitle(
        "Custo de Memória para Cada Cenário de Teste",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_title(
    ...
    ax.set_xlabel("Cenários de teste")
    ax.set_ylabel("Memória usada (MB)")
...
def main() -> None:
    ...
    print(f"Cenários no gráfico: {len(df)}")
    for reason in excluded:
        print(f"  excluído -> {reason}")
