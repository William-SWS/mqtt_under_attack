#!/usr/bin/env python3
"""
Fronteira de Pareto empírica: desempenho vs mediana do tempo de inferência por linha.
...
requisições, dividida pelo número de linhas do dataset (18925).

Gera em paretto/:
  - pareto_frontier_median_per_line_lowvariance.csv
  - pareto_frontier_median_per_line_lowvariance.png
"""
...
OUTPUT_CSV = PARETTO_DIR / "pareto_frontier_median_per_line_lowvariance.csv"
OUTPUT_PNG = PARETTO_DIR / "pareto_frontier_median_per_line_lowvariance.png"

# ponytail: confirmado via wc -l nos datasets de teste
N_ROWS = 18925
    ...
    for i, b in enumerate(data):
        for j, a in enumerate(data):
            ...
            if (
                ...
                and a["time_per_line_ms"] <= b["time_per_line_ms"]
                and (a["combined"] > b["combined"] or a["time_per_line_ms"] < b["time_per_line_ms"])
            ):
                b["dominated"] = True
...
def plot_frontier(data: list[dict], path: Path) -> None:
    ...
    if dominated:
        ax.scatter(
            [d["time_per_line_ms"] for d in dominated],
    ...
    if frontier:
        ax.scatter(
            [d["time_per_line_ms"] for d in frontier],
    ...
    if len(frontier) >= 2:
        fs = sorted(frontier, key=lambda d: d["time_per_line_ms"])
        ax.plot(
            [d["time_per_line_ms"] for d in fs],
    ...
    all_models = sorted(data, key=lambda d: d["time_per_line_ms"])
    y_max = max(d["combined"] * 100 for d in all_models)
    for idx, d in enumerate(all_models):
        label = d["model_id"].replace("lowvariance_", "")
        text = f"{label}\n({d['time_per_line_ms']:.3f} ms; {d['combined'] * 100:.2f}%)"
        xy = (d["time_per_line_ms"], d["combined"] * 100)
    ...
    ax.set_title(
        ...
        "e do tempo de inferência por registro",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_xlabel("Tempo de inferência por linha (ms)")
...
def main() -> None:
    ...
    for model_id in LOWVARIANCE_MODELS:
        ...
        time_per_line = b["time_ms_p50"] / N_ROWS
        data.append(
            {
                "model_id": model_id,
                "time_ms_p50": b["time_ms_p50"],
                "time_per_line_ms": time_per_line,
                "accuracy": b["accuracy"],
    ...
    print(f"\nFronteira de Pareto — mediana por linha ({len(data)} modelos):")
    for d in sorted(data, key=lambda x: x["time_per_line_ms"]):
        ...
        print(
            ...
            f"por_linha={d['time_per_line_ms']:>10.6f} ms  "
